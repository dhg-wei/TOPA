...
from llama import Transformer_llama3,Tokenizer_llama3,ModelArgs_llama3
...
def LLaMA_VQA(args, **kwargs):
    ...
    if args.llama3:
        model_args: ModelArgs = ModelArgs_llama3(max_seq_len=args.max_seq_len, max_batch_size=32, adapter_len=args.adapter_len, adapter_layer=args.adapter_layer, **params)
    else:
    ...
    if args.llama3:
        model_llama_vqa = Transformer_llama3(model_args, args)
    else:
    ...
    for name, param in model_llama_vqa.named_parameters():
        ...
        if ('gate' in name) or ('adapter' in name) or ('temporal_emb' in name) or ('visual_proj' in name) or ('query_tokens' in name):
