...
class BaseDataset(Dataset):
    ...
    def _get_caption_token(self, text, answer):

        vaq_id, vaq_prefix_index, vaq_video_start = self.tokenizer.encode_videocap(text=text, max_feats=self.max_feats, split=self.split, answer_mapping=self.answer_mapping, answer=answer)
        vaq_id = [torch.tensor(v_id, dtype=torch.int64) for v_id in vaq_id]
        ...
        text_id = {'vaq': vaq_padding_text_id}
        label = {'vaq': vaq_label}
        video_start = {'vaq': vaq_video_start}
        video_index = {'vaq': vaq_video_index}
        label_mask = {'vaq': vaq_label_mask}
        return text_id, label, video_start, video_index, label_mask
    
    def _get_openvqa_token(self, text, answer):

        vaq_id, vaq_prefix_index, vaq_video_start = self.tokenizer.encode_openvqa(text=text, max_feats=self.max_feats, split=self.split, answer_mapping=self.answer_mapping, answer=answer)
        vaq_id = [torch.tensor(v_id, dtype=torch.int64) for v_id in vaq_id]
        
        vaq_padding_text_id = self._get_padding_id(vaq_id)
 
        vaq_label = copy.deepcopy(vaq_padding_text_id)
        if type(vaq_prefix_index)==list:
            for i,index_ in enumerate(vaq_prefix_index):
                vaq_label[i, :index_] = -1
        else:
            vaq_label[:, :vaq_prefix_index] = -1
        vaq_label_mask = vaq_label.ge(0)
        vaq_label[~vaq_label_mask] = 0
