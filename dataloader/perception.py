...
class PerceptionTest(BaseDataset):
    def __init__(self, args=None, tokenizer=None, split='train'):
        super().__init__(args, tokenizer, split)
        self.data = pd.read_csv(f'./data/perception/{split}.csv')

        self.answer_mapping = {0: '(A)', 1: '(B)', 2: '(C)'}
        self.num_options = 3
        self.qtype_mapping = {'CH': 1, 'CW': 2, 'TN': 3, 'TC': 4, 'TP': 5, 'DL': 6, 'DC': 7, 'DO': 8}
        self.features_path = '../../EgoSchema/benchmarking/FrozenBilm/features_perception_test_2fps/'
